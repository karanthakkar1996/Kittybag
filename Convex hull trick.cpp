...
{   /* line = a*x + b
    ...
    bool bad(std::set<line_segment>::iterator it){
        return hasPrev(it) && hasNext(it)&& (    isMax && bad(*std::prev(it), *it, *std::next(it))|| !isMax && bad(*std::next(it), *it, *std::prev(it)) );
    }
    ...
    retll evaluate(retlls x) const{
        if(slopes.empty())return (retll)1e18;
...
// If you need index of line as well and smaller index line for same evaluation
...
{   /* line = a*x + b
    ...
    struct line_segment{
        ...
        bool operator<(const line_segment& l2) const{
            if (l2.type == line){
                if(this->a == l2.a)
                    return this->idx > l2.idx;
                return this->a > l2.a;
            }
            if (l2.type == maxQuery){
                if(this->xLeft == l2.val)
                    return this->idx > l2.idx;
    ...
    void insert_Line(fft a, fft b,llu idxxxx){
        line_segment l3 = line_segment(a, b);
        l3.idx = idxxxx;
        auto it = slopes.lower_bound(l3);
        if (it!=slopes.end() && dont_meet(*it, l3)){
    ...
    pair<llu,llu> evaluate(retlls x) const{
        if(slopes.empty())assert(0);
        line_segment q;q.val = x;
        q.type = isMax ? line_segment::Type::maxQuery : line_segment::Type::minQuery;
        auto bestLine = slopes.lower_bound(q);
        if (isMax) --bestLine;
        return {bestLine->idx,(llu)(bestLine->valueAt(x)+0.5)};
    }
};
